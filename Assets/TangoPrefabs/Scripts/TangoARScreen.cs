...
{   
    ...
    private static Vector2 _GetUnityUvBasedOnRotation(Vector2 uv, OrientationManager.Rotation colorCamerRDisplay)
    {
        ...
        case OrientationManager.Rotation.ROTATION_180:
            return new Vector2(1.0f - uv.x, 1.0f - uv.y);
    ...
    /// <param name="colorCamerRDisplay">Rotation enum from color camera to display.</param>
    private static void _MeshUpdateForIntrinsics(Mesh mesh, float uOffset, float vOffset,
                                                 OrientationManager.Rotation colorCamerRDisplay)
    {
        ...
        for (int i = 0; i < 4; ++i)
        {
            uvs[i] = _GetUnityUvBasedOnRotation(uvs[i], colorCamerRDisplay);
        }
    ...
    /// <summary>
    /// Set color camera render plane and the render camera.
    /// </summary>
                                     ...
                                     OrientationManager.Rotation colorCameraRotation)
    {
        ...
        if (alignedIntrinsics.width != 0 && alignedIntrinsics.height != 0)
        {
            ...
            _MeshUpdateForIntrinsics(m_screenSpaceMesh, m_uOffset, m_vOffset, rotation);
