...
namespace Tango
{
    ...
    internal static class TangoPrefabInspectorHelper
    {
                                                           ...
                                                           bool shouldUsePermissions)
        {
            ...
            DisplayExperimentalDriftCorrectionWarning(tangoApplication, shouldUsePermissions);
        ...
        /// <summary>
        /// Displays a warning regarding the relation between experimental drift correction 
        /// and use of Area Description poses.
        /// </summary>
        /// <param name="tangoApplication">Reference to Tango Application.</param>
        /// <param name="usingAreaDescriptionFrame">If set to <c>true</c>, caller is using area description frame.</param>
        public static void DisplayExperimentalDriftCorrectionWarning(TangoApplication tangoApplication,
                                                                     bool usingAreaDescriptionFrame)
        {
            if (tangoApplication.m_enableDriftCorrection)
            {
                string driftCorrectionBaseString = "Experimental drift correction is enabled in TangoApplication.\n" 
                                                   + "Drift-corrected poses use Area Description pose but don't become" 
                                                   + "valid until some time after start-up.";
                if (usingAreaDescriptionFrame)
                {
                    EditorGUILayout.HelpBox(driftCorrectionBaseString +
                                            "\n\nFor a good user experience, you may have to write custom code " +
                                            "to define behavior during the warm-up period.",
                                            MessageType.Warning);
                }
                else
                {
                    EditorGUILayout.HelpBox(driftCorrectionBaseString +
                                            "\n\nConsider checking \"Use Area Description Pose\".",
                                            MessageType.Warning);
                }
            }
        }
