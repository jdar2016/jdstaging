    ...
    public class TangoApplication : MonoBehaviour
    {
        ...
        private void _InitializeMotionTracking(string uuid)
        {
            ...
            if (m_enableDriftCorrection && 
                (usedUUID || m_areaDescriptionLearningMode))
            {
                Debug.LogError("Experimental drift correction cannot be used with area"
                               + "learning or with an area description loaded.");
                m_tangoConfig.SetBool(TangoConfig.Keys.EXPERIMENTAL_ENABLE_DRIFT_CORRECTION_BOOL, false);
            }
            else
            {
                m_tangoConfig.SetBool(TangoConfig.Keys.EXPERIMENTAL_ENABLE_DRIFT_CORRECTION_BOOL, m_enableDriftCorrection);
            }
